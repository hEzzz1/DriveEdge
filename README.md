...
## 1. 项目说明
DriveEdge 是疲劳驾驶与分心检测边缘端项目。当前仓库的主落地实现是 `edge-app` 安卓应用，已经收敛为“本地推理 + 事件上报”链路，不再默认启用云端推理。
...
5. 本地录制 MP4。
6. 本地质检图片落盘。
7. 边缘原始风险事件上报到 `DriveServer /api/v1/events`。
8. 弱网下本地队列缓存与网络恢复后重传。
9. 页面内展示事件上报状态。
...
6. 将边缘端检测到的原始风险事件编码并上报到 `DriveServer /api/v1/events`。
7. 弱网下进入本地队列并按退避策略重试，网络恢复后继续补传。
8. 页面状态区显示事件上报结果。
9. 质检目录保存 `probe/raw/model/overlay/local_detected` 等图片。
...
2. 当前事件上报接口为 `DriveServer /api/v1/events`。
3. `docs/` 中部分旧文档仍保留历史内容，主要用于回溯，不代表当前运行主路径。
